...
import crypto.srtp.retained.InitiatorRetainedSecretsCalculator;
import crypto.srtp.retained.RetainedSecrets;
import crypto.srtp.retained.RetainedSecretsCalculator;
...
public class ZRTPInitiatorSocket extends ZRTPSocket {
  ...
  protected void handleDH(HandshakePacket packet) throws InvalidPacketException {
    ...
    case KA_TYPE_EC25:
      foreignDH  = new crypto.srtp.EC25DHPartOnePacket(packet, true);
  ...
  protected int getKeyAgreementType() {
    if (foreignHello == null)
      ...
      /// Check with the server authentication here and declar the value as it will return a key
      //agreement with the called (as a handshake signal)
    PhoneClientId foreignClientId = new PhoneClientId(foreignHello.getClientId());
