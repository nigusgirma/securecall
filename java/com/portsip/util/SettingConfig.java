...
public class SettingConfig {
	...
	static final String PORTSIP ="PORTSIP_NETWORKTYPE";
...
//		Editor editor = mPrefrence.edit();
	...
	public void db_Info(String name, String password,String phone){
		//PORTSIP_USERNAME=name;
		iName=name;
		iPassword=password;
		iPnumber=phone;
	}
	public static UserInfo getUserInfo(Context context){
		...
		SharedPreferences mPrefrence = context
				.getSharedPreferences(PRE_NAME,Context.MODE_PRIVATE);
		String stringitem = mPrefrence.getString(PORTSIP_USERNAME, infor.getUserName());//Phone number will be here
		infor.setUserName(stringitem);
		stringitem = mPrefrence.getString(PORTSIP_USERPASSWORD, infor.getUserPassword());
		infor.setUserPwd(stringitem);
		stringitem = mPrefrence.getString(PORTSIP_USEDISPALYNAME, infor.getUserDisplayName());// the name of the user will be here
		infor.setUserDisplayName(stringitem);
		stringitem = mPrefrence.getString(PORTSIP_USERDOMAIN, "");		
		infor.setUserDomain(stringitem);
		stringitem = mPrefrence.getString(PORTSIP_SIPSERVER, "callman.cloudapp.net");
		infor.setSipServer(stringitem);
		int port = mPrefrence.getInt(PORTSIP_SIPSERVERPORT, 5060);
		infor.setSipPort(port);
		stringitem = mPrefrence.getString(PORTSIP_STUNSVR, "callman.cloudapp.net");
		infor.setStunServer(stringitem);
		port = mPrefrence.getInt(PORTSIP_STUNPORT, 3478);
